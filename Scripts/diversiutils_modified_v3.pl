...
if (($help)&&!($help)||!($bam)||!($ref)){
  ...
  print "| |  | | \\ \\ / / _ \\ '__/ __| |  | |/ _ \\ / _ \\| / __|  modified by XD V3\n";
...
#open (AACOOLECTION, ">$stub\_aa_details.txt");
...
foreach my $target (@targets){
  if (!$refseq{$target}){# check that reference is present in the reference file
    ...
    print "Calculating variation frequencies for $target:\n";
    $increment=0;
                  ...
                  if ($codreg{$target}{$prot}{"Beg"}<$codreg{$target}{$prot}{"End"}){
					  ...
					  my ($aasite,$mod);
						...
						if ($site>=$codreg{$target}{$prot}{"Beg"} && $site<=$codreg{$target}{$prot}{"End"} && $mod==1 && $i<(scalar(@bases)-2)){
						  ...
						  #print AACOOLECTION $aasite."\t".$aafreq{$target}{$prot}{$aasite}{"AAcoverage"}."\t".$aafreq{$target}{$prot}{$aasite}{"RefAA"}."\t$target\t$prot\t$rcodon\t$raa\t$qcodon\t$qaa\n";
...
print OUT "Sample\tChr\tPosition\tRefBase\tCoverage\tAvQual\tAcnt\tApval\tCcnt\tCpval\tTcnt\tTpval\tGcnt\tGpval\tentropy(base e)\tNonRefCnt\tCntTs\tCntTv\tOrderOfNucs\tstrandbias\tins_cnt\tins_mode\tdel_cnt\tdel_mode\tNcnt\n";
...
if ($orfs){
  ...
  open (AAALLAA, ">$stub\_AA_all_AA.txt")||die "can't open $stub\_AA_all_AA.txt\n";
  open (AAALLCONDON, ">$stub\_AA_all_condon.txt")||die "can't open $stub\_AA_all_condon.txt\n";
  # the position of the mutation
  ...
  print AAALLAA "Chr\tProtein\tAAPosition\tTopAArank\tRefAA\tRefSite\tRefCodon\tTopAA\tTopAAcnt\tAAcoverage\n";
  print AAALLCONDON "Chr\tProtein\tAAPosition\tAAcount\tRefAA\tRefSite\tRefCodon\tTopCodon\tTopCodoncnt\tAAcoverage\n";
        ...
        for (my $aasite=1; $aasite<=$aacnt; $aasite++){
         if (defined $aafreq{$target}{$prot}{$aasite}{"RefAA"}){
          ...
          foreach my $aa (sort { $aaorder{$target}{$prot}{$aasite}{$b} <=> $aaorder{$target}{$prot}{$aasite}{$a} } keys %{$aaorder{$target}{$prot}{$aasite}}) {
            ...
            $topAAscount++;
            print AAALLAA "$target\t$prot\t$aasite\t";
            print AAALLAA "$topAAscount\t";
            print AAALLAA $aafreq{$target}{$prot}{$aasite}{"RefAA"}."\t";
            print AAALLAA $aafreq{$target}{$prot}{$aasite}{"RefSite"}."\t";
            print AAALLAA $aafreq{$target}{$prot}{$aasite}{"RefCodon"}."\t";
            print AAALLAA "$aa\t$aaorder{$target}{$prot}{$aasite}{$aa}\t";
            print AAALLAA $aafreq{$target}{$prot}{$aasite}{"AAcoverage"}."\n";
          }          
          ...
          foreach my $codon (sort { $codonfreq{$target}{$prot}{$aasite}{$b} <=> $codonfreq{$target}{$prot}{$aasite}{$a} } keys %{$codonfreq{$target}{$prot}{$aasite}}) {
            ...
            $topCodonscount++;
            print AAALLCONDON "$target\t$prot\t$aasite\t";
            print AAALLCONDON "$topCodonscount\t";
            print AAALLCONDON $aafreq{$target}{$prot}{$aasite}{"RefAA"}."\t";
            print AAALLCONDON $aafreq{$target}{$prot}{$aasite}{"RefSite"}."\t";
            print AAALLCONDON $aafreq{$target}{$prot}{$aasite}{"RefCodon"}."\t";
            print AAALLCONDON "$codon\t$codonfreq{$target}{$prot}{$aasite}{$codon}\t";
            print AAALLCONDON $aafreq{$target}{$prot}{$aasite}{"AAcoverage"}."\n";
          }
